...
*)
    ...
    PROMPT='%{$fg_bold[blue]%}[${USER}@${HOSTNAME}] %(!.#.$) ${RESET}'
    #RPROMPT='${RESET}${WHITE}[${BLUE}%(5~,%-2~/.../%2~,%~)% ${WHITE}]${RESET}'
    RPROMPT='${RESET}${WHITE}%(5~,%-2~/.../%2~,%~)%#${RESET}'
    ...
    function zle-line-init zle-keymap-select {
      case $KEYMAP in
        ...
        PROMPT='${GREEN}%(5~,%-2~/../%2~,%~)${RESET}
%{$fg_bold[cray]%}[${USER}@${HOSTNAME}] %(!.#.$) ${RESET}'
        ;;
        main|viins)
        #PROMPT="%{$fg_bold[blue]%}${USER}@%m ${RESET}${WHITE}${POH} ${RESET}"
        PROMPT='${GREEN}%(5~,%-2~/../%2~,%~)${RESET}
%{$fg_bold[blue]%}[${USER}@${HOSTNAME}] %(!.#.$) ${RESET}'
        ;;
      esac
      zle reset-prompt
    }
...
# C-M-h でチートシートを表示する
cheat-sheet () { zle -M "`cat ~/Git/dotfiles/.zsh/cheat-sheet`" }
zle -N cheat-sheet
...
## alias設定
#
[ -f ~/Git/dotfiles/.zshrc.alias ] && source ~/Git/dotfiles/.zshrc.alias

case "${OSTYPE}" in
# Mac(Unix)
darwin*)
    # ここに設定
    [ -f ~/Git/dotfiles/.zshrc.osx ] && source ~/Git/dotfiles/.zshrc.osx
    ;;
# Linux
linux*)
    # ここに設定
    [ -f ~/Git/dotfiles/.zshrc.linux ] && source ~/Git/dotfiles/.zshrc.linux
    ;;
esac
